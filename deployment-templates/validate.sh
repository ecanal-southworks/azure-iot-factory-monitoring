#!/bin/bash
# Since validation for linked templates is not available, this script validates them individually
...
az deployment group validate --resource-group="$RESOURCE_GROUP_NAME" --mode="Incremental" --subscription="$SUBSCRIPTION_NAME" --template-file="$TEMPLATE_PATH" --parameters="$PARAMETERS_PATH"
